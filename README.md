...
- ✅ **全自动 Cookie 管理**：用即刷 + 周期刷新 + 失效自动救 + 抽风重试四层机制；失败 10 秒快速自愈，OneBot 恢复无需干预，全程无感知。
...
| `timeout` | int | 否 | HTTP请求超时时间（秒），默认10。注意：从 OneBot 获取 Cookie 的请求使用独立的 5 秒快速超时（v1.4.4 起），不受此项影响。 |
