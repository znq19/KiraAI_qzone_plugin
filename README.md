...
- ✅ **自动配图（带概率控制）**：发说说时如果未指定图片，插件会以 `auto_publish_image_prob` 的概率从当前会话的最近图片中挑选一张，统一控制配图频率。
...
| `auto_publish_image_prob` | float | 否 | **统一控制自动配图概率**（0-1），默认0.5。无论后台模式还是指令模式，发说说时若未指定图片且随机数低于此值，则会自动配图。 |
...
- **自动配图概率**：通过 `auto_publish_image_prob` 配置项调整，无需改代码。
...
- **评论回复的 @ 功能**：回复评论时，插件会自动在内容前加上 `回复 @昵称：`，以实现子回复效果，但可能会形成重复@昵称效果（已在 v1.35 尝试优化）。
