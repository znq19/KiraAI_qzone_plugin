...
- ✅ **灵活的任务目标**：定时任务可以从多个群聊或私聊中随机选择一个执行，避免单一目标；指令自带场合信息（群名/昵称），AI 不会搞错任务发生的会话。
...
- **工作原理**：定时时间到达后，插件会向随机选择的群聊或私聊发送一条特殊指令（例如“【定时任务】请根据最近聊天发布一条说说”），指令会自动附带场合信息（如「当前场合：群『绿岛酒吧』188395693」或「与『周武』的私聊」），AI 在该会话中收到消息后，会像普通对话一样处理，自主生成内容并发布/评论/回复。发布成功后，AI 可以根据你的提示词决定是否调用记忆工具记录。
