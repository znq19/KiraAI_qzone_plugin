...
- ✅ **记忆系统集成**：通过 KiraOS 的记忆工具，AI 能记住自己发过什么，后续对话可以自然提起。
- ✅ **自动配图**：发说说时如果未指定图片，插件会自动从当前会话的最近图片中挑选一张。
- ✅ **自动刷新 Cookie**：通过 LLOneBot 的 HTTP 接口自动获取最新 Cookie，无需手动复制，配合后台保活机制可长期稳定运行。
...
3. **OneBot 协议实现**（如 LLOneBot 或 NapCat）已开启 HTTP 服务，并记下地址和端口（通常为 `127.0.0.1:3000`）。
4. 你的 QQ 号（用于获取 Cookie）以及一个或多个用于接收定时任务的群聊/私聊。
...
> **说明**：如果你希望使用手动浏览器获取 Cookie 的备用方式，可以额外安装 `selenium` 和 `webdriver-manager`，但非必需且实测难以起效。

各依赖的作用：
- `apscheduler`：定时任务管理
- `beautifulsoup4`：解析 HTML，用于获取说说详情
- `json5`：解析非标准 JSON 响应
- `httpx`：异步 HTTP 请求（用于与 LLOneBot 通信及保活）
...
| 配置项 | 类型 | 必填 | 说明 |
|--------|------|------|------|
| `cookies_str` | text | 否 | QQ空间登录 Cookie（如果启用自动刷新则无需填写）。 |
| `qq_account` | string | 否 | QQ账号（用于备用自动登录，可选）。 |
| `qq_password` | string | 否 | QQ密码（用于备用自动登录，可选）。 |
| `auto_refresh_cookie` | boolean | 否 | 是否启用自动刷新 Cookie（通过 LLOneBot 获取），默认 `true`。 |
| `master_ids` | string | 否 | 主人QQ号，多个用英文逗号分隔。只有这些QQ号才能使用QQ空间相关工具（发说说、评论、点赞等）。 |
| `timeout` | int | 否 | HTTP请求超时时间（秒），默认10。 |
| `temp_dir` | string | 否 | 临时文件存放目录，默认`data/temp`。 |
| `llonebot_host` | string | 否 | OneBot HTTP服务地址，默认`127.0.0.1`。 |
| `llonebot_port` | int | 否 | OneBot HTTP服务端口，默认3000。 |
| `access_token` | string | 否 | OneBot 访问令牌（如果有）。 |
...
| `like_when_comment` | boolean | 否 | 自动评论时是否同时点赞，默认true。 |
...
插件现在通过 LLOneBot /NapCat的 HTTP 接口直接获取当前登录 QQ 的明文 Cookie，无需任何手动操作。只需在配置中填写正确的 `llonebot_host`、`llonebot_port` 和 `access_token`，并启用 `auto_refresh_cookie`，插件启动时会自动获取最新 Cookie。

若获取失败，会回退到使用配置文件中手动填写的 `cookies_str`。同时，插件内置了**保活任务**，每30分钟向 QQ 空间发送一次访客请求，使 Cookie 长期有效（最长24小时）。即使保活失败，也会自动重新从 LLOneBot 获取新 Cookie。

**注意**：LLOneBot /NapCat必须已登录你的 QQ 账号，并且开启了 HTTP 服务。
...
本插件默认假设 OneBot HTTP 服务由 **LLOneBot** 提供，API 路径和参数与 LLOneBot 一致。如果你使用的是 **NapCat**，理论上也能直接使用，如果不行则可能需要进行少量修改，因为 NapCat 的 API 端点可能略有不同。

### 主要区别
- LLOneBot 使用 `/get_cookies` 等端点，而 NapCat 可能使用相同的端点（OneBot 标准），但有时需要加上 `/` 前缀或其他差异。
- 如果你的 NapCat 配置了不同的端口或需要鉴权，只需在插件配置中修改 `llonebot_host`、`llonebot_port` 和 `access_token` 即可。
- 如果遇到“获取历史失败”的错误，可能是 API 路径不对。你可以在 `main.py` 中找到相关请求（如 `_fetch_group_history`），将 `api` 变量改为 NapCat 对应的端点，例如：
  ```python
  api = "get_group_msg_history"   # 可能 NapCat 需要 "/get_group_msg_history"
  # 如果不行，尝试在 base_url 后加上 "/" 或去掉 "/"
  ```
  一般来说，OneBot 标准是统一的，无需修改。如果确实有问题，请查阅 NapCat 文档确认 API 路径。

### 简单排查步骤
1. 用浏览器访问 `http://127.0.0.1:3000/get_cookies`（假设端口是3000），看是否返回错误信息。如果返回 404，说明路径不对。
2. 查阅 NapCat 文档，找到正确的获取 Cookie 接口地址。
3. 在 `main.py` 中搜索 `_get_cookie_from_llonebot`，将请求的 URL 路径替换为正确的。
...
**Q：我用的是 NapCat，无法获取历史消息怎么办？**  
A：请参考上面 **兼容 NapCat 的修改说明** 部分，检查 API 路径是否正确。如果仍有问题，可联系开发者提供日志。（虽然可能开发者太菜也无法解决QAQ）
...
- **点赞功能可能失效**：由于 QQ 空间点赞接口可能发生变化，目前点赞功能在实际测试中无法成功但不影响其他功能使用。
- **评论回复的 @ 功能**：回复评论时，插件会自动在内容前加上 `回复 @昵称：`，以实现子回复效果，但可能会形成重复@昵称效果。
...
本项目同样MIT 协议开源，可自由使用和修改。

---

**如果遇到问题，欢迎在 KiraAI 交流群讨论，并自行修改。因为本人技术实在太差，还请谅解，祝玩的愉快和让 Kira 记住点点滴滴~**
