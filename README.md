...
- ✅ **自动配图（后台模式专属）**：后台直接生成模式发说说时，以 `auto_publish_image_prob` 的概率从目标会话最近图片中配图；群聊指令模式由 AI 自行决定。
- ✅ **全自动 Cookie 管理（新机制）**：彻底废弃手动复制 Cookie 和长时间保活任务。**每次调用空间功能时，插件会强制从已连接的适配器获取最新 Cookie**，确保 Cookie 始终有效；同时修复了旧版中因保活任务导致的 `Unclosed client session` 错误。
...
> **说明**：v1.35 已彻底移除旧的无用依赖（如 selenium），仅保留必要的库。
...
| 配置项 | 类型 | 必填 | 说明 |
|--------|------|------|------|
| `cookies_str` | text | 否 | QQ空间登录 Cookie（**如果启用自动刷新则无需填写**）。v1.35 推荐使用自动刷新。 |
| `qq_ada` | string | 否 | QQ 适配器名称。如果你的 KiraAI 有多个适配器（例如同时连接 LLOneBot 和 NapCat），请指定平台为 QQ 的适配器名称；留空则插件自动查找第一个平台为 QQ 的适配器。 |
| `auto_refresh_cookie` | boolean | 否 | 是否启用自动刷新 Cookie（通过已连接的适配器获取），默认 `true`。开启后，每次调用空间功能前都会强制获取最新 Cookie。 |
...
## 🔐 Cookie 自动获取机制（v1.35 全新升级）

插件现在**完全自动化和即时化**管理 Cookie：
- **每次调用空间功能前**（发说说、评论、点赞等），插件都会通过已连接的 QQ 适配器（如 LLOneBot/NapCat）直接获取当前登录账号的 Cookie，确保 Cookie 始终最新有效。
- **首次启动时**也会尝试获取 Cookie，如果适配器尚未就绪，插件会进入待机状态，并在后续首次调用时重试。
...
- **配图控制**：由 AI 自行决定是否配图（`auto_publish_image_prob` 不生效）。
...
- **配图控制**：受 `auto_publish_image_prob` 概率控制。
- **优点**：完全静默，不影响群聊。
...
### QQ空间工具
你拥有以下工具，可以操作我的QQ空间：
- `qzone_publish(text, image_urls)`：发布一条说说。如果需要配图，插件会自动从当前会话的最近图片中选择（有概率控制），你不必提供 `image_urls`。
...
A：可能是适配器尚未完全就绪或网络问题。插件会在每次调用空间功能时重试，通常一次失败后第二次就会成功。如果持续失败，请检查 QQ 适配器是否正常工作。
